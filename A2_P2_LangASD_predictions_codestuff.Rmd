...
```{r libraries and nice functions}
...
df_trainy <- df_train %>% 
  select(Child.ID, CHI_MLU, Visit, Ethnicity, Diagnosis, Gender, Age, MOT_MLU, CHI_LUstd, types_MOT, types_CHI, tokens_MOT, tokens_CHI, verbalIQ1, nonVerbalIQ1,  Socialization1) %>% 
  drop_na()

full_model <- lmerTest::lmer(CHI_MLU ~ (Visit + Ethnicity + Diagnosis + Gender + Age + MOT_MLU + CHI_LUstd + types_MOT + types_CHI + tokens_MOT + tokens_CHI + verbalIQ1 + nonVerbalIQ1 + Socialization1)^2 + (1|Child.ID), data=df_trainy, na.action = na.fail)

# Model selection 
library(lmerTest)

besty_model <- lmerTest::lmer(CHI_MLU ~ Visit + Ethnicity + Diagnosis + Gender + Age + Socialization + 
    ...
    Ethnicity:types_CHI + Ethnicity:tokens_MOT + Diagnosis:Socialization + Gender:Age + 
    ...
    types_MOT:types_CHI + types_CHI:tokens_MOT + tokens_MOT:tokens_CHI, data=df_train)
...
#- optional: predictions are never certain, can you identify the uncertainty of the predictions? (e.g. google predictinterval())
predictInterval(besty_model, data_test, allow.new.levels=TRUE, interval="predict")
...
result_df <- tibble(cv_num=numeric(), rmse=numeric())
i <- 1
for (fold in folds) {
  ...
  preds <- predict(m1, testdat, allow.new.levels=T) %>% 
    nicify_predictions()
...
#- Now try to find the best possible predictive model of ChildMLU, that is, the one that produces the best cross-validated results.
model_list <- list("m1" = m1, "m2" = m2, "m3" = m3)


result_df <- tibble(model_name=character(), cv_num=numeric(), rmse=numeric())
...
0.5 vs 15.5 (!) --> wtf, way, way, way more
...
summarised_cv <- result_df %>% 
  group_by(model_name) %>% 
  summarise(cv_rmse = mean(rmse))

map_df(names(model_list), get_relevant_metrics) %>% 
  arrange(AIC) %>% 
  left_join(summarised_cv, by = c("name" = "model_name"))
```

Note: m3 is super overfit and it what the fuck
